...
class Iansan(Character):
    ...
    def apply_dependent_buffs(self, all_hits: dict["Character", dict[str, SkillHit]]) -> None:
        ...
        #
        # 읽는 쪽은 current_atk()가 아니라 convertible_atk()다 — 버프 제공량 계산이므로
        # %-파생 지분은 재료에서 빠진다(스탯을 읽는 버프의 공통 규칙).
        #
        # 출력이 atk_from_pct_share인 것은 그 규칙의 결과이면서 동시에 자기 참조를 끊는다.
        # 이 엔진은 순서를 버리고 지연 평가로 값을 정하므로 같은 슬롯을 읽고 쓰면 값이
        # 정해지지 않는데(얀사 자신을 필드 위로 고르면 바로 그 경우다), 재료와 출력이 다른
        # 필드라 고리가 안 생긴다. 최종 공격력(current_atk)에는 그대로 들어가므로 스탯
        # 시트와 얀사 자신의 피해 계산에는 정상적으로 실린다.

...
class VortexVanquisher(Weapon):
    """관홍의 창 (Vortex Vanquisher) | 장병기 | 5성
    패시브: 금빛의 옥 · 제군의 길
    ...
    def apply_passive(self, all_hits, wearer) -> None:
        ...
        # 묻는다 — 참봉의 칼날·무공의 검·속세의 자물쇠(같은 패시브의 검·양손검·법구
        # 버전)와 같은 꼴.
        stacks = ask_int(
            "[관홍의 창] 「금빛의 옥」 스택 수 (공격 명중 시 획득, 0.3초당 최대 1회, 최대 5)",
            0, 5,
        )
        if not stacks:
            return
        ...
        # 로테이션이 정하는 상태라 묻는다.
        multiplier = 2.0 if ask_bool("[관홍의 창] 보호막 존재 여부 (「금빛의 옥」 공격력 증가 2배)") else 1.0

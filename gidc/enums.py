...
class CharacterTrait(Enum):
    ...
    # 인원수로 정해진 달빛 징조 레벨을 한 칸 더 올린다(린네아 「서식지 조사」).
    # MOONSIGN과 나눠 두는 이유는 「기여하는가」와 「올리는가」가 다른 질문이기 때문이다 —
    # 보유자를 인원수에 두 번 세면 달빛 징조 캐릭터 1명짜리 파티가 정원 2명으로 보인다.
    MOONSIGN_ELEVATION = "달빛 징조 상승"
    ...
    LUNAR_CRYSTALLIZE_CONVERTER = "달결정 전환"   # 콜롬비나·린네아·자백(미구현)
...
class ArtifactSet(Enum):
    # ── 5성 세트 (최신순) ──────────────────────────────────────────
    SCARLET_PROOF                           = "핏빛 증표"
    HEART_OF_THE_FURNACE                    = "용광로가 빚은 심장"
